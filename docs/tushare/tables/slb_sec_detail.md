# `slb_sec_detail` 标准化表契约

- 功能：转融券交易明细
- PostgreSQL 表：`tushare_norm_slb_sec_detail`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=slb_sec_detail`）
- REST：`GET /api/v1/data/datasets/slb_sec_detail/records`
- 接口 REST：`GET /api/v1/data/interfaces/slb_sec_detail/records`
...
| `name` | `str` | `TEXT` | 股票名称 |
| `tenor` | `str` | `TEXT` | 期 限(天) |
| `fee_rate` | `float` | `NUMERIC` | 融出费率(%) |
| `lent_qnt` | `float` | `NUMERIC` | 转融券融出数量(万股) |
