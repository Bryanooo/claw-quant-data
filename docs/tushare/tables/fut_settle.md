...
- PostgreSQL 表：`tushare_norm_fut_settle`
- 契约版本：`1`
...
- 当前数据视图：`tushare_current_fut_settle`
- 原始数据表：`tushare_raw_record`（`api_name=fut_settle`）
- REST：`GET /api/v1/data/datasets/fut_settle/records`
- 接口 REST：`GET /api/v1/data/interfaces/fut_settle/records`
...
| `settle` | `float` | `NUMERIC` | 结算价 |
| `trading_fee_rate` | `float` | `NUMERIC` | 交易手续费率 |
| `trading_fee` | `float` | `NUMERIC` | 交易手续费 |
| `delivery_fee` | `float` | `NUMERIC` | 交割手续费 |
| `b_hedging_margin_rate` | `float` | `NUMERIC` | 买套保交易保证金率 |
| `s_hedging_margin_rate` | `float` | `NUMERIC` | 卖套保交易保证金率 |
| `long_margin_rate` | `float` | `NUMERIC` | 买投机交易保证金率 |
| `short_margin_rate` | `float` | `NUMERIC` | 卖投机交易保证金率 |
| `offset_today_fee` | `float` | `NUMERIC` | 平今仓手续率 |
