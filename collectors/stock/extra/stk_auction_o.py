...
logger = logging.getLogger("collector.stk_auction_o")

_FIELDS = ["ts_code", "trade_date", "close", "open", "high", "low", "vol", "amount", "vwap"]
_PK = ["ts_code", "trade_date"]


class StkAuctionOCollector:
    INTERFACE_NAME = "stk_auction_o"
    ...
    def fetch(self, **params):
        return self.pro.stk_auction_o(**params)
    ...
    def collect_all_history(self, start_date="20200101", end_date="20260513"):
        ...
        while d <= e:
            ...
            d += timedelta(days=1)
            time.sleep(0.3)
        logger.info(f"✅ stk_auction_o: 历史补齐 {total} 行")
        return total
