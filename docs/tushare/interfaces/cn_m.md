...
- 分类：宏观经济/国内宏观/金融/货币供应量
- 功能：获取货币供应量之月度数据
- Token 权限：**有权限**（接口返回成功）
- 官方权限要求：用户积累600积分可以使用，具体请参阅 积分获取办法
- 官方文档：[doc 242](https://tushare.pro/document/2?doc_id=242)
...
强类型标准表 `tushare_norm_cn_m`。标准表字段、类型、日期列、系统血缘字段和
稳定性规则见[标准化表契约](../tables/cn_m.md)。转换失败记录进入
