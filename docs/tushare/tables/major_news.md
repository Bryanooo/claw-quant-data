...
- PostgreSQL 表：`tushare_norm_major_news`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=major_news`）
- REST：`GET /api/v1/data/datasets/major_news/records`
- 接口 REST：`GET /api/v1/data/interfaces/major_news/records`
...
|---|---|---|---|
| `title` | `str` | `TEXT` | 标题 |
| `content` | `str` | `TEXT` | 内容 (默认不显示，需要在fields里指定) |
| `pub_time` | `str` | `TIMESTAMP WITHOUT TIME ZONE` | 发布时间 |
| `src` | `str` | `TEXT` | 来源网站 |
