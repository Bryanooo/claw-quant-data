...
- 功能：查询组合的成分列表
- PostgreSQL 表：`tushare_norm_p_get`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=p_get`）
- REST：`GET /api/v1/data/datasets/p_get/records`
- 接口 REST：`GET /api/v1/data/interfaces/p_get/records`
...
|---|---|---|---|
| `id` | `int` | `BIGINT` | 编号 |
| `ts_code` | `str` | `TEXT` | 成分代码 |
| `ts_type` | `str` | `TEXT` | 成份类型（用户自定义类型，比如按行业、按概念板块或其他） |
| `name` | `str` | `TEXT` | 名称 |
| `desc` | `str` | `TEXT` | 描述 |
| `weight` | `float` | `NUMERIC` | 权重 |
