...
- 功能：获取ETF基准指数列表信息
- PostgreSQL 表：`tushare_norm_etf_index`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=etf_index`）
- REST：`GET /api/v1/data/datasets/etf_index/records`
- 接口 REST：`GET /api/v1/data/interfaces/etf_index/records`
...
| `indx_name` | `str` | `TEXT` | 指数全称 |
| `indx_csname` | `str` | `TEXT` | 指数简称 |
| `pub_party_name` | `str` | `TEXT` | 指数发布机构 |
| `pub_date` | `str` | `DATE` | 指数发布日期 |
| `base_date` | `str` | `DATE` | 指数基日 |
| `bp` | `float` | `NUMERIC` | 指数基点(点) |
| `adj_circle` | `str` | `TEXT` | 指数成份证券调整周期 |
