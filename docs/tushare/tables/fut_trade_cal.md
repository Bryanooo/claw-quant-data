# `fut_trade_cal` 标准化表契约

- 功能：获取各大期货交易所交易日历数据
- PostgreSQL 表：`tushare_norm_fut_trade_cal`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=fut_trade_cal`）
- REST：`GET /api/v1/data/datasets/fut_trade_cal/records`
- 接口 REST：`GET /api/v1/data/interfaces/fut_trade_cal/records`
...
|---|---|---|---|
| `exchange` | `str` | `TEXT` | 交易所 同参数部分描述 |
| `cal_date` | `str` | `DATE` | 日历日期 |
| `is_open` | `int` | `BIGINT` | 是否交易 0休市 1交易 |
| `pretrade_date` | `str` | `DATE` | 上一个交易日 |
