...
- PostgreSQL 表：`tushare_norm_fut_holding`
- 契约版本：`1`
- 技术主键：`_record_hash`（原始 payload SHA-256）
- 主日期字段：`trade_date`
- 必填身份字段：`trade_date, symbol`
- 业务身份字段：`trade_date, symbol, broker`
- 业务身份可信度：`contract_reviewed`
- 当前数据视图：`tushare_current_fut_holding`
- 原始数据表：`tushare_raw_record`（`api_name=fut_holding`）
- REST：`GET /api/v1/data/datasets/fut_holding/records`
- 接口 REST：`GET /api/v1/data/interfaces/fut_holding/records`
...
| `symbol` | `str` | `TEXT` | 合约代码或类型 |
| `broker` | `str` | `TEXT` | 期货公司会员简称 |
| `vol` | `int` | `BIGINT` | 成交量 |
| `vol_chg` | `int` | `BIGINT` | 成交量变化 |
| `long_hld` | `int` | `BIGINT` | 持买仓量 |
| `long_chg` | `int` | `BIGINT` | 持买仓量变化 |
| `short_hld` | `int` | `BIGINT` | 持卖仓量 |
| `short_chg` | `int` | `BIGINT` | 持卖仓量变化 |
