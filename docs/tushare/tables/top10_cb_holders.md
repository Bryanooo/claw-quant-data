# `top10_cb_holders` 标准化表契约

- 功能：获取可转债前十大持有人
- PostgreSQL 表：`tushare_norm_top10_cb_holders`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=top10_cb_holders`）
- REST：`GET /api/v1/data/datasets/top10_cb_holders/records`
- 接口 REST：`GET /api/v1/data/interfaces/top10_cb_holders/records`
...
| `end_date` | `str` | `DATE` | 报告期 |
| `holder_rank` | `int` | `BIGINT` | 持有排名 |
| `holder_name` | `str` | `TEXT` | 持有人名称 |
| `hold_amount` | `float` | `NUMERIC` | 持有数量(万张) |
| `hold_ratio` | `float` | `NUMERIC` | 持有比例(%) |
