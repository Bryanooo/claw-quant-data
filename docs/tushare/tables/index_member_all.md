# `index_member_all` 标准化表契约

- 功能：按三级分类提取申万行业成分，可提供某个分类的所有成分，也可按股票代码提取所属分类，参数灵活
- PostgreSQL 表：`tushare_norm_index_member_all`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=index_member_all`）
- REST：`GET /api/v1/data/datasets/index_member_all/records`
- 接口 REST：`GET /api/v1/data/interfaces/index_member_all/records`
...
|---|---|---|---|
| `l1_code` | `str` | `TEXT` | 一级行业代码 |
| `l1_name` | `str` | `TEXT` | 一级行业名称 |
| `l2_code` | `str` | `TEXT` | 二级行业代码 |
| `l2_name` | `str` | `TEXT` | 二级行业名称 |
| `l3_code` | `str` | `TEXT` | 三级行业代码 |
| `l3_name` | `str` | `TEXT` | 三级行业名称 |
| `ts_code` | `str` | `TEXT` | 成分股票代码 |
| `name` | `str` | `TEXT` | 成分股票名称 |
| `in_date` | `str` | `DATE` | 纳入日期 |
| `out_date` | `str` | `DATE` | 剔除日期 |
| `is_new` | `str` | `TEXT` | 是否最新Y是N否 |
