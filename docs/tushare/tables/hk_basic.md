...
- PostgreSQL 表：`tushare_norm_hk_basic`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=hk_basic`）
- REST：`GET /api/v1/data/datasets/hk_basic/records`
- 接口 REST：`GET /api/v1/data/interfaces/hk_basic/records`
...
| `name` | `str` | `TEXT` | 股票简称 |
| `fullname` | `str` | `TEXT` | 公司全称 |
| `enname` | `str` | `TEXT` | 英文名称 |
| `cn_spell` | `str` | `TEXT` | 拼音 |
| `market` | `str` | `TEXT` | 市场类别 |
| `list_status` | `str` | `TEXT` | 上市状态 |
| `list_date` | `str` | `DATE` | 上市日期 |
| `delist_date` | `str` | `DATE` | 退市日期 |
| `trade_unit` | `float` | `NUMERIC` | 交易单位 |
| `isin` | `str` | `TEXT` | ISIN代码 |
| `curr_type` | `str` | `TEXT` | 货币代码 |
