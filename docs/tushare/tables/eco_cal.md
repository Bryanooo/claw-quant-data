...
- PostgreSQL 表：`tushare_norm_eco_cal`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=eco_cal`）
- REST：`GET /api/v1/data/datasets/eco_cal/records`
- 接口 REST：`GET /api/v1/data/interfaces/eco_cal/records`
...
|---|---|---|---|
| `date` | `str` | `DATE` | 日期 |
| `time` | `str` | `TIME WITHOUT TIME ZONE` | 时间 |
| `currency` | `str` | `TEXT` | 货币代码 |
| `country` | `str` | `TEXT` | 国家 |
| `event` | `str` | `TEXT` | 经济事件 |
| `value` | `str` | `TEXT` | 今值 |
| `pre_value` | `str` | `TEXT` | 前值 |
| `fore_value` | `str` | `TEXT` | 预测值 |
...
- 上游 `time=试验性` 表示具体时刻未确定，标准化为 `NULL`；原始文字仍保留在原始表。
