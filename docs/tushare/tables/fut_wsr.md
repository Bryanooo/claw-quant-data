...
- PostgreSQL 表：`tushare_norm_fut_wsr`
- 契约版本：`1`
- 技术主键：`_record_hash`（原始 payload SHA-256）
- 主日期字段：`trade_date`
- 必填身份字段：`trade_date, symbol`
- 业务身份字段：`trade_date, symbol, warehouse, wh_id, grade, brand, place`
- 业务身份可信度：`contract_reviewed`
- 当前数据视图：`tushare_current_fut_wsr`
- 原始数据表：`tushare_raw_record`（`api_name=fut_wsr`）
- REST：`GET /api/v1/data/datasets/fut_wsr/records`
- 接口 REST：`GET /api/v1/data/interfaces/fut_wsr/records`
...
| `symbol` | `str` | `TEXT` | 产品代码 |
| `fut_name` | `str` | `TEXT` | 产品名称 |
| `warehouse` | `str` | `TEXT` | 仓库名称 |
| `wh_id` | `str` | `TEXT` | 仓库编号 |
| `pre_vol` | `int` | `BIGINT` | 昨日仓单量 |
| `vol` | `int` | `BIGINT` | 今日仓单量 |
| `vol_chg` | `int` | `BIGINT` | 增减量 |
| `area` | `str` | `TEXT` | 地区 |
| `year` | `str` | `TEXT` | 年度 |
| `grade` | `str` | `TEXT` | 等级 |
| `brand` | `str` | `TEXT` | 品牌 |
