# `sz_daily_info` 标准化表契约

- 功能：获取深圳市场每日交易概况
- PostgreSQL 表：`tushare_norm_sz_daily_info`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=sz_daily_info`）
- REST：`GET /api/v1/data/datasets/sz_daily_info/records`
- 接口 REST：`GET /api/v1/data/interfaces/sz_daily_info/records`
...
|---|---|---|---|
| `trade_date` | `str` | `DATE` |  |
| `ts_code` | `str` | `TEXT` | 市场类型 |
| `count` | `int` | `BIGINT` | 股票个数 |
| `amount` | `float` | `NUMERIC` | 成交金额 |
| `vol` | `int` | `BIGINT` | 成交量 |
| `total_share` | `float` | `NUMERIC` | 总股本 |
| `total_mv` | `float` | `NUMERIC` | 总市值 |
| `float_share` | `float` | `NUMERIC` | 流通股票 |
