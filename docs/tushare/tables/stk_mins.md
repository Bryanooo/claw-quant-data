...
- PostgreSQL 表：`tushare_norm_stk_mins`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=stk_mins`）
- REST：`GET /api/v1/data/datasets/stk_mins/records`
- 接口 REST：`GET /api/v1/data/interfaces/stk_mins/records`
...
| `trade_time` | `str` | `TIMESTAMP WITHOUT TIME ZONE` | 交易时间 |
| `open` | `float` | `NUMERIC` | 开盘价 |
| `close` | `float` | `NUMERIC` | 收盘价 |
| `high` | `float` | `NUMERIC` | 最高价 |
| `low` | `float` | `NUMERIC` | 最低价 |
| `vol` | `int` | `BIGINT` | 成交量(股) |
| `amount` | `float` | `NUMERIC` | 成交金额（元） |
