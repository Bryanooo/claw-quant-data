# `fund_portfolio` 标准化表契约

- 功能：获取公募基金持仓数据，季度更新
- PostgreSQL 表：`tushare_norm_fund_portfolio`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=fund_portfolio`）
- REST：`GET /api/v1/data/datasets/fund_portfolio/records`
- 接口 REST：`GET /api/v1/data/interfaces/fund_portfolio/records`
...
|---|---|---|---|
| `ts_code` | `str` | `TEXT` | TS基金代码 |
| `ann_date` | `str` | `DATE` | 公告日期 |
| `end_date` | `str` | `DATE` | 截止日期 |
| `symbol` | `str` | `TEXT` | 股票代码 |
| `mkv` | `float` | `NUMERIC` | 持有股票市值(元) |
| `amount` | `float` | `NUMERIC` | 持有股票数量（股） |
| `stk_mkv_ratio` | `float` | `NUMERIC` | 占股票市值比 |
| `stk_float_ratio` | `float` | `NUMERIC` | 占流通股本比例 |
