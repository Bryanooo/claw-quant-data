...
- 功能：获取可转债评级历史记录
- PostgreSQL 表：`tushare_norm_cb_rating`
- 契约版本：`1`
- 技术主键：`_record_hash`（原始 payload SHA-256）
- 主日期字段：`ann_date`
- 必填身份字段：`ts_code`
- 业务身份字段：`ts_code, rating_date, rating_com_name, rating_type`
- 业务身份可信度：`contract_reviewed`
- 当前数据视图：`tushare_current_cb_rating`
- 原始数据表：`tushare_raw_record`（`api_name=cb_rating`）
- REST：`GET /api/v1/data/datasets/cb_rating/records`
- 接口 REST：`GET /api/v1/data/interfaces/cb_rating/records`
...
| `ann_date` | `str` | `DATE` | 评级发布日期 |
| `rating_date` | `str` | `DATE` | 评级日期 |
| `rating_com_name` | `str` | `TEXT` | 评级机构 |
| `rating_way` | `str` | `TEXT` | 评级方式 |
| `rating_type` | `str` | `TEXT` | 评级类别 |
| `rating` | `str` | `TEXT` | 信用等级 |
| `rating_outlook` | `str` | `TEXT` | 评级展望 |
