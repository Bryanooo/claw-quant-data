...
- 功能：获取美国短期国债利率数据
- PostgreSQL 表：`tushare_norm_us_tbr`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=us_tbr`）
- REST：`GET /api/v1/data/datasets/us_tbr/records`
- 接口 REST：`GET /api/v1/data/interfaces/us_tbr/records`
...
|---|---|---|---|
| `date` | `str` | `DATE` | 日期 |
| `w4_bd` | `float` | `NUMERIC` | 4周银行折现收益率 |
| `w4_ce` | `float` | `NUMERIC` | 4周票面利率 |
| `w8_bd` | `float` | `NUMERIC` | 8周银行折现收益率 |
| `w8_ce` | `float` | `NUMERIC` | 8周票面利率 |
| `w13_bd` | `float` | `NUMERIC` | 13周银行折现收益率 |
| `w13_ce` | `float` | `NUMERIC` | 13周票面利率 |
| `w17_bd` | `float` | `NUMERIC` | 17周银行折现收益率（数据从20221019开始） |
| `w17_ce` | `float` | `NUMERIC` | 17周票面利率（数据从20221019开始） |
| `w26_bd` | `float` | `NUMERIC` | 26周银行折现收益率 |
| `w26_ce` | `float` | `NUMERIC` | 26周票面利率 |
| `w52_bd` | `float` | `NUMERIC` | 52周银行折现收益率 |
| `w52_ce` | `float` | `NUMERIC` | 52周票面利率 |
