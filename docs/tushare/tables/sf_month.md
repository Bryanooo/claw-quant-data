...
- 功能：获取月度社会融资数据
- PostgreSQL 表：`tushare_norm_sf_month`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=sf_month`）
- REST：`GET /api/v1/data/datasets/sf_month/records`
- 接口 REST：`GET /api/v1/data/interfaces/sf_month/records`
...
|---|---|---|---|
| `month` | `str` | `TEXT` | 月度 |
| `inc_month` | `float` | `NUMERIC` | 社融增量当月值（亿元） |
| `inc_cumval` | `float` | `NUMERIC` | 社融增量累计值（亿元） |
| `stk_endval` | `float` | `NUMERIC` | 社融存量期末值（万亿元） |
