...
- 功能：获取期权合约信息
- Token 权限：**有权限**（接口返回成功）
- 官方权限要求：用户需要至少5000积分可以调取，具体请参阅 积分获取办法
- 官方文档：[doc 158](https://tushare.pro/document/2?doc_id=158)
...
强类型标准表 `tushare_norm_opt_basic`。标准表字段、类型、日期列、系统血缘字段和
稳定性规则见[标准化表契约](../tables/opt_basic.md)。转换失败记录进入
