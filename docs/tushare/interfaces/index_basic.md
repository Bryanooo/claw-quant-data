...
该接口由专用采集器写入规范化业务表 `index_basic`，主键为 `ts_code`。自动周任务
按市场分区；CSI 已超过官方 8,000 行单次上限，因此继续按官方 `category` 枚举
分区。系统还会用 CSI 无过滤发现样本校验类别并集，任一分区触顶、参数未生效或
样本未被并集覆盖都会失败，所有分区验证完成后才原子替换快照。2026-09-08
真实验证为 11,707 行，其中 CSI 8,941 行。
