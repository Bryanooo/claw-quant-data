# `ci_index_member` 标准化表契约

- 功能：按三级分类提取中信行业成分，可提供某个分类的所有成分，也可按股票代码提取所属分类，参数灵活
- PostgreSQL 表：`tushare_norm_ci_index_member`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=ci_index_member`）
- REST：`GET /api/v1/data/datasets/ci_index_member/records`
- 接口 REST：`GET /api/v1/data/interfaces/ci_index_member/records`
