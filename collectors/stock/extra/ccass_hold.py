...
logger = logging.getLogger("collector.ccass_hold")

_FIELDS = ["trade_date", "ts_code", "name", "shareholding", "hold_nums", "hold_ratio"]
_PK = ["ts_code", "trade_date"]


class CcassHoldCollector:
    INTERFACE_NAME = "ccass_hold"
    ...
    def fetch(self, **params):
        return self.pro.ccass_hold(**params)
    ...
    def collect_all_history(self, start_date="20200101", end_date="20260513"):
        ...
        while d <= e:
            ...
            d += timedelta(days=1)
            time.sleep(0.3)
        logger.info(f"✅ ccass_hold: 历史补齐 {total} 行")
        return total
