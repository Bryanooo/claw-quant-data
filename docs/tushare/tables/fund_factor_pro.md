# `fund_factor_pro` 标准化表契约

- 功能：获取场内基金每日技术面因子数据，用于跟踪场内基金当前走势情况，数据由Tushare社区自产，覆盖全历史；输出参数_bfq表示不复权，描述中说明了因子的默认传参，如需要特殊参数或者更多因子可以联系管理员评估
- PostgreSQL 表：`tushare_norm_fund_factor_pro`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=fund_factor_pro`）
- REST：`GET /api/v1/data/datasets/fund_factor_pro/records`
- 接口 REST：`GET /api/v1/data/interfaces/fund_factor_pro/records`
