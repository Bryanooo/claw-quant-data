...
- PostgreSQL 表：`tushare_norm_fund_manager`
- 契约版本：`1`
- 技术主键：`_record_hash`（原始 payload SHA-256）
- 主日期字段：`ann_date`
- 必填身份字段：`ts_code`
- 业务身份字段：`ts_code, name, begin_date`
- 业务身份可信度：`contract_reviewed`
- 当前数据视图：`tushare_current_fund_manager`
- 原始数据表：`tushare_raw_record`（`api_name=fund_manager`）
- REST：`GET /api/v1/data/datasets/fund_manager/records`
- 接口 REST：`GET /api/v1/data/interfaces/fund_manager/records`
...
| `name` | `str` | `TEXT` | 基金经理姓名 |
| `gender` | `str` | `TEXT` | 性别 F:女 M:男 |
| `birth_year` | `str` | `TEXT` | 出生年份 |
| `edu` | `str` | `TEXT` | 学历 |
| `nationality` | `str` | `TEXT` | 国籍 |
| `begin_date` | `str` | `DATE` | 任职日期 |
| `end_date` | `str` | `DATE` | 离任日期 |
| `resume` | `str` | `TEXT` | 简历 |
