# `index_classify` 标准化表契约

- 功能：获取申万行业分类，可以获取申万2014年版本（28个一级分类，104个二级分类，227个三级分类）和2021年本版（31个一级分类，134个二级分类，346个三级分类）列表信息
- PostgreSQL 表：`tushare_norm_index_classify`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=index_classify`）
- REST：`GET /api/v1/data/datasets/index_classify/records`
- 接口 REST：`GET /api/v1/data/interfaces/index_classify/records`
...
|---|---|---|---|
| `index_code` | `str` | `TEXT` | 指数代码 |
| `industry_name` | `str` | `TEXT` | 行业名称 |
| `parent_code` | `str` | `TEXT` | 父级代码 |
| `level` | `str` | `TEXT` | 行业层级 |
| `industry_code` | `str` | `TEXT` | 行业代码 |
| `is_pub` | `str` | `TEXT` | 是否发布了指数 |
| `src` | `str` | `TEXT` | 行业分类（SW申万） |
