...
数据日历采用更窄且可证明的口径：185 个自动编排接口中，49 个数据集具备定期覆盖
审计规则，只有其中 34 个具备可按 A 股交易日逐日严格判断的强日频契约。周、月、季、
事件型、无独立可靠交易日历以及多个接口共享数据表的对象，不进入“每日 34 个”的
分母。历史日期显示“已有数据”表示物理分区确实存在，但当时适用的数据集范围尚未
完全建模或未按当前规则审计；“严格完成”才表示当日 34 个强日频数据集全部通过当前
规则。任务失败只作为旁证展示，不能把已通过数据审计的日期染红，也不能在非交易日
制造数据缺失。
...
4. 目录历史：宏观利率/账户统计按年分区，高数量转融通按月分区并分页穷尽；`full`
   计划 v6 同时回填多来源新闻、卖方研报、股东持仓、回购解禁、基金持仓，以及
   股票/ETF日线与复权因子、THS板块和SGE现货逐交易日历史；
...
1. 七个阶段全部物化并执行结束，不存在 `queued/running/failed` 步骤；
2. 每个采集步骤均为 `complete`，或具有严格验证证据的允许空分区；
...
滚动升级期间，旧 Worker 如果领取到由更高处理器版本创建的任务，会无损让出任务，
不消耗重试次数。`install.sh` 会强制重建全部应用容器，并校验 API、Scheduler、
三个 Worker 资源池和 Auditor 使用同一镜像，避免部分容器仍运行旧代码。
...
`parent_job_id`，不再把批次父子关系和人工重试混为一谈。批次叶子重试是例外：
它会将同一个独立叶子实例重新入队，因此实例编号保持不变。控制台的“执行实例”账本
可以查看这两种重试方式，并在提交重试后实时轮询新实例状态。

控制台按六种不同对象组织信息，彼此不能混作同一个统计口径：

| 视图 | 对象与问题 | 权威时间/状态口径 |
|---|---|---|
| 今日交付 | 今日应该交付哪些数据、是否按时完成 | 计划业务日、目标数据日、最终截止时间 |
| 投资日历 | 哪些宏观数据或衍生品事件会在某日发布/发生 | 上游事件日期与合约真实最后交割日 |
| 全量采集活动（扇出） | 一个接口跨冻结实体全集的分页编排进度 | 活动周期、实体游标、全部叶子与最终审计 |
| 执行记录 | 每一次真实入队、执行、自动尝试和人工重试 | 不可变原始状态，并附加当前是否已恢复 |
| 数据日历 | 某个数据日期的数据最终是否可用 | 业务表 `data_date`、物理分区和当前规则审计 |
| 待处理事项 | 当前仍未解决且需要行动的问题 | 有效失败、逾期、缺失、截面不完整和服务健康 |

失败执行记录不会因后续恢复而删除。API 为失败记录附加 `resolution_state`、
`resolution_reason` 以及恢复实例、活动或覆盖审计编号；“待处理事项”只统计
`unresolved`，已经被后续证据恢复的记录仅保留在执行账本中。
...
指数基础快照不会再调用一次无参数接口：系统按市场拆分，并将已经超过官方
8,000 行上限的 CSI 市场继续按官方类别拆分，所有分区验证完成后才原子替换
`index_basic`。`ths_member` 也不再把 6,000 行上限响应当全量；它每周冻结完整
`ths_index` 板块宇宙，为每个板块创建独立、可重试、可审计的子任务。
...
覆盖规则自身有版本号。规则口径修正后，系统只对已有严格传输完整性证据的旧失败
结果自动重跑数据库审计，不重复请求上游。`index_daily` 的预期宇宙按官方契约排除
申万指数（由 `sw_daily` 负责），并为海外/跨市场指数在当地休市日不发布预留经过
真实相邻交易日验证的截面容差；分页截断仍会保持失败关闭。
由于 `index_basic` 不提供完整历史失效日期，当前指数宇宙的截面阈值仅用于最近
120 天；更早历史以逐日范围/分页耗尽和日期分区存在性作为可证明的完整性契约。
同理，`stock_basic` 不能还原三十年前 Tushare 实际保留的财务报表宇宙；四类核心
财务数据只在最近五年执行公司截面比例校验，更早报告期仍严格要求分页穷尽、报告期
回显，并要求分区存在或该精确报告期具备严格验证空证据；近五年空结果仍失败关闭，
避免把历史口径差异误报成缺失。
...
- 在“投资日历”按月查看 GDP、PMI、通胀、就业、央行和股指期货交割事件；点击日期查看时间、前值、预测、实际值、合约与来源；
- 每项今日计划显示计划时间、目标数据周期、来源、严格完成证据和交付截止时间；
- 数据日历明确展示 185 个自动接口、49 个定期审计数据集和 34 个强日频数据集三层口径；
- 数据日期状态仅由物理分区和当前规则覆盖审计决定，任务异常单独作为执行旁证展示；
- 未到发布时间显示为等待，只有执行失败、覆盖不完整或超过截止时间才进入真实异常；
- 当日未验证任务按持久化任务 ID 对齐交付截止时间，截止前等待复采、截止后才升级；
- 左侧导航按“运营 / 执行与审计 / 资产与配置”分组，分别查看今日交付、投资日历、数据日历、待处理事项、全量采集活动、执行记录、接口清单和数据资产；
- “执行记录”查询 PostgreSQL 中完整的不可变任务账本，使用服务端分页；失败记录明确分为当前未恢复、已恢复/非当前问题和全部失败历史；
- “全量采集活动（扇出）”表示一个接口跨实体全集、跨分页推进的编排容器，不等同于今日任务；可从活动直接钻取分页批次及其全部执行实例，再查看实例内部的自动尝试；
- 在“待处理事项”统一查看当前真实缺失、空表、未首采和完整性未知；已恢复失败不占当前异常数；
...
- 对失败任务执行重试；
- 查看最近执行实例、实例内自动尝试次数、Worker、时间、错误原因以及人工重试链；
- 幂等生成最近日、周、月和季度补采任务。
...
| `GET /api/v1/research/stocks/{ts_code}/technicals` | K线/缺口、趋势/动量、量能异常/价量四象限/突破与背离/换手率/日线VWAP代理、下行风险、基准风险、六类枢轴、斐波那契回撤、缠论候选结构和图表序列 |
...
就绪检查。当前机器可读 v1 基线为
59 项：51 项已服务、3 项可由既有数据补采后增加、5 项仍需新数据源或解除额度约束。
