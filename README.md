...
| 暂不安全自动运行 | 22 个 |
| 当前有正行数证据 | 179 个接口 |
| 当前无正行数证据 | 21 个接口（自动接口仅 `p_list`，未建用户组合时合法为空） |
...
3. **周期全量扇出**：为 17 个已验证接口冻结实体宇宙，分批生成可恢复的
   日、周或月度活动。
...
| 原因 | 数量 | 处理方式 |
|---|---:|---|
| 需要按股票、基金、转债或指数代码扇出 | 11 | 已提供受控批次，尚未全部纳入固定周期 |
| 官方目录已无完整输入契约 | 4 | 仅允许显式调用，不猜测参数 |
| 必填枚举或已实测单分区触顶 | 4 | 等待明确枚举、依赖来源和更细分区规则 |
| `hk_daily`、`stk_mins` 极低频限流 | 2 | 不进入常规队列，避免拖垮 Worker |
| 依赖另一个接口提供扇出清单 | 1 | `p_get` 已接入依赖检查 |

这 22 个接口不是“没有实现”，而是不适合按固定周期直接全量调用。其中仍有一部分
...
`full` 初始化会在独立的“全量扇出”阶段自动执行 18 类安全基线：除转债利率/
评级、期货合约、中信/申万行业成分和全股票质押统计外，还覆盖必须按本地冻结实体
宇宙拆分的基金净值/持仓、筹码、股东、审计意见、指数权重与周月频复权数据。
...
分区的标为 `not_applicable`。每天自动审计仍只运行16条已验证规则；其他日期型
...
对 `stock_daily`、`stock_daily_basic`、`moneyflow`、`stock_limit` 这类能够按单日
幂等重跑的数据集，Auditor 会把 `missing` 或 `partial` 分区限量提交到采集队列；
`index_weekly` 和 `index_monthly` 使用全市场日期分区及 offset 穷尽做安全修复；
...
| `GET /api/v1/coverage/datasets/{name}/partitions` | 查询存在、截面不完整、缺失和仅观察的日期明细 |
