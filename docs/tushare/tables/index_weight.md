...
- PostgreSQL 表：`tushare_norm_index_weight`
- 契约版本：`1`
- 技术主键：`_record_hash`（原始 payload SHA-256）
- 主日期字段：`trade_date`
- 必填身份字段：`trade_date, index_code`
- 业务身份字段：`trade_date, index_code`
- 业务身份可信度：`heuristic`
- 当前数据视图：未创建（业务身份仍为启发式，REST 保留全部 payload 版本）
- 原始数据表：`tushare_raw_record`（`api_name=index_weight`）
- REST：`GET /api/v1/data/datasets/index_weight/records`
- 接口 REST：`GET /api/v1/data/interfaces/index_weight/records`
