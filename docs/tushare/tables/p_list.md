...
- 功能：便于用户在基于AI应用或Vibe Coding过程中产生的选股结果形成长期记忆，Tushare提供了自选股组合功能。本接口可以查询你的自选组合信息，不加参数查询出所以自定义组合
- PostgreSQL 表：`tushare_norm_p_list`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=p_list`）
- REST：`GET /api/v1/data/datasets/p_list/records`
- 接口 REST：`GET /api/v1/data/interfaces/p_list/records`
...
| `name` | `str` | `TEXT` | 名称 |
| `desc` | `str` | `TEXT` | 描述 |
| `create_time` | `datetime` | `TIMESTAMP WITHOUT TIME ZONE` | 创建时间 |
| `update_time` | `datetime` | `TIMESTAMP WITHOUT TIME ZONE` | 修改时间 |
