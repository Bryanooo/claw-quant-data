# `stk_week_month_adj` 标准化表契约

- 功能：股票周/月线行情(复权--每日更新)
- PostgreSQL 表：`tushare_norm_stk_week_month_adj`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=stk_week_month_adj`）
- REST：`GET /api/v1/data/datasets/stk_week_month_adj/records`
- 接口 REST：`GET /api/v1/data/interfaces/stk_week_month_adj/records`
...
| `pre_close` | `float` | `NUMERIC` | 上一(周/月)收盘价【除权价，前复权】 |
| `open_qfq` | `float` | `NUMERIC` | 前复权(周/月)开盘价 |
| `high_qfq` | `float` | `NUMERIC` | 前复权(周/月)最高价 |
| `low_qfq` | `float` | `NUMERIC` | 前复权(周/月)最低价 |
| `close_qfq` | `float` | `NUMERIC` | 前复权(周/月)收盘价 |
| `open_hfq` | `float` | `NUMERIC` | 后复权(周/月)开盘价 |
| `high_hfq` | `float` | `NUMERIC` | 后复权(周/月)最高价 |
| `low_hfq` | `float` | `NUMERIC` | 后复权(周/月)最低价 |
| `close_hfq` | `float` | `NUMERIC` | 后复权(周/月)收盘价 |
| `vol` | `float` | `NUMERIC` | (周/月)成交量 |
| `amount` | `float` | `NUMERIC` | (周/月)成交额 |
| `change` | `float` | `NUMERIC` | (周/月)涨跌额 |
| `pct_chg` | `float` | `NUMERIC` | (周/月)涨跌幅 【基于除权后的昨收计算的涨跌幅：（今收-除权昨收）/除权昨收 】 |
