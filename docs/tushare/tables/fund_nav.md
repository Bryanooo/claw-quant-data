...
- 功能：获取公募基金净值数据
- PostgreSQL 表：`tushare_norm_fund_nav`
- 契约版本：`1`
- 技术主键：`_record_hash`（原始 payload SHA-256）
- 主日期字段：`ann_date`
- 必填身份字段：`nav_date, ts_code`
- 业务身份字段：`nav_date, ts_code`
- 业务身份可信度：`heuristic`
- 当前数据视图：未创建（业务身份仍为启发式，REST 保留全部 payload 版本）
- 原始数据表：`tushare_raw_record`（`api_name=fund_nav`）
- REST：`GET /api/v1/data/datasets/fund_nav/records`
- 接口 REST：`GET /api/v1/data/interfaces/fund_nav/records`
...
| `nav_date` | `str` | `DATE` | 净值日期 |
| `unit_nav` | `float` | `NUMERIC` | 单位净值 |
| `accum_nav` | `float` | `NUMERIC` | 累计净值 |
| `accum_div` | `float` | `NUMERIC` | 累计分红 |
| `net_asset` | `float` | `NUMERIC` | 资产净值 |
| `total_netasset` | `float` | `NUMERIC` | 合计资产净值 |
| `adj_nav` | `float` | `NUMERIC` | 复权单位净值 |
