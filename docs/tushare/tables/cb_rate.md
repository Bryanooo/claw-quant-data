...
- 功能：获取可转债票面利率
- PostgreSQL 表：`tushare_norm_cb_rate`
- 契约版本：`1`
- 技术主键：`_record_hash`（原始 payload SHA-256）
- 主日期字段：`无`
- 必填身份字段：`ts_code`
- 业务身份字段：`ts_code, rate_start_date, rate_end_date`
...
| `rate_freq` | `int` | `BIGINT` | 付息频率(次/年) |
| `rate_start_date` | `str` | `DATE` | 付息开始日期 |
| `rate_end_date` | `str` | `DATE` | 付息结束日期 |
