...
- 功能：Libor拆借利率
- PostgreSQL 表：`tushare_norm_libor`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=libor`）
- REST：`GET /api/v1/data/datasets/libor/records`
- 接口 REST：`GET /api/v1/data/interfaces/libor/records`
...
|---|---|---|---|
| `date` | `str` | `DATE` | 日期 |
| `curr_type` | `str` | `TEXT` | 货币 |
| `on` | `float` | `NUMERIC` | 隔夜 |
| `1w` | `float` | `NUMERIC` | 1周 |
| `1m` | `float` | `NUMERIC` | 1个月 |
| `2m` | `float` | `NUMERIC` | 2个月 |
| `3m` | `float` | `NUMERIC` | 3个月 |
| `6m` | `float` | `NUMERIC` | 6个月 |
| `12m` | `float` | `NUMERIC` | 12个月 |
