...
- 功能：获取国民经济之GDP数据
- PostgreSQL 表：`tushare_norm_cn_gdp`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=cn_gdp`）
- REST：`GET /api/v1/data/datasets/cn_gdp/records`
- 接口 REST：`GET /api/v1/data/interfaces/cn_gdp/records`
...
|---|---|---|---|
| `quarter` | `str` | `TEXT` | 季度 |
| `gdp` | `float` | `NUMERIC` | GDP累计值（亿元） |
| `gdp_yoy` | `float` | `NUMERIC` | 当季同比增速（%） |
| `pi` | `float` | `NUMERIC` | 第一产业累计值（亿元） |
| `pi_yoy` | `float` | `NUMERIC` | 第一产业同比增速（%） |
| `si` | `float` | `NUMERIC` | 第二产业累计值（亿元） |
| `si_yoy` | `float` | `NUMERIC` | 第二产业同比增速（%） |
| `ti` | `float` | `NUMERIC` | 第三产业累计值（亿元） |
| `ti_yoy` | `float` | `NUMERIC` | 第三产业同比增速（%） |
