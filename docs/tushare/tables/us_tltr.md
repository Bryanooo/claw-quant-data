...
- 功能：国债长期利率
- PostgreSQL 表：`tushare_norm_us_tltr`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=us_tltr`）
- REST：`GET /api/v1/data/datasets/us_tltr/records`
- 接口 REST：`GET /api/v1/data/interfaces/us_tltr/records`
...
|---|---|---|---|
| `date` | `str` | `DATE` | 日期 |
| `ltc` | `float` | `NUMERIC` | 收益率 LT COMPOSITE (>10 Yrs) |
| `cmt` | `float` | `NUMERIC` | 20年期CMT利率(TREASURY 20-Yr CMT) |
| `e_factor` | `float` | `NUMERIC` | 外推因子EXTRAPOLATION FACTOR |
