...
- 官方文档：[doc 172](https://tushare.pro/document/2?doc_id=172)
- HTTP：`POST http://api.tushare.pro`
- claw-quant：已实现；代码：[collectors/index/monthly.py:IndexMonthlyCollector](../../../collectors/index/monthly.py)
...
该接口由专用采集器写入 `index_monthly` 规范化业务表，业务主键为
`(ts_code, trade_date)`。自动任务按上一个完整月的最后交易日生成
`trade_date` 分区，使用 `limit=1000` 和递增 `offset` 拉取全市场；只有返回短页
证明分页耗尽后才记录 `complete`。重复页或达到 `max_pages` 会记录为不完整并重试，
不会把 1000 行整数上限误判成完整结果。

真实验证：`trade_date=20260831` 共 10 页、9,364 行，任务证据为
