...
- 功能：期货日线行情数据
- PostgreSQL 表：`tushare_norm_fut_daily`
- 契约版本：`1`
...
- 当前数据视图：`tushare_current_fut_daily`
- 原始数据表：`tushare_raw_record`（`api_name=fut_daily`）
- REST：`GET /api/v1/data/datasets/fut_daily/records`
- 接口 REST：`GET /api/v1/data/interfaces/fut_daily/records`
...
|---|---|---|---|
| `ts_code` | `str` | `TEXT` | TS合约代码 |
| `trade_date` | `str` | `DATE` | 交易日期 |
| `pre_close` | `float` | `NUMERIC` | 昨收盘价 |
| `pre_settle` | `float` | `NUMERIC` | 昨结算价 |
| `open` | `float` | `NUMERIC` | 开盘价 |
| `high` | `float` | `NUMERIC` | 最高价 |
| `low` | `float` | `NUMERIC` | 最低价 |
| `close` | `float` | `NUMERIC` | 收盘价 |
| `settle` | `float` | `NUMERIC` | 结算价 |
| `change1` | `float` | `NUMERIC` | 涨跌1 收盘价-昨结算价 |
| `change2` | `float` | `NUMERIC` | 涨跌2 结算价-昨结算价 |
| `vol` | `float` | `NUMERIC` | 成交量(手) |
| `amount` | `float` | `NUMERIC` | 成交金额(万元) |
| `oi` | `float` | `NUMERIC` | 持仓量(手) |
| `oi_chg` | `float` | `NUMERIC` | 持仓量变化 |
| `delv_settle` | `float` | `NUMERIC` | 交割结算价 |
