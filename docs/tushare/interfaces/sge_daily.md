...
- 分类：现货数据
- 功能：获取上海黄金交易所现货合约日线行情
- Token 权限：**有权限**（接口返回成功）
- 官方权限要求：用户积2000积分可调取，具体请参阅 积分获取办法 注：数据由当日9:00至15:30的交易和前一日夜盘的20:00至2:30数据构成，成交量和成交金额为双向计量。
- 官方文档：[doc 285](https://tushare.pro/document/2?doc_id=285)
- HTTP：`POST http://api.tushare.pro`
- claw-quant：已实现；代码：[collectors/sge/sge_daily.py:SgeDailyCollector](../../../collectors/sge/sge_daily.py)
