...
- 功能：获取可转债行情
- PostgreSQL 表：`tushare_norm_cb_daily`
- 契约版本：`1`
...
- 当前数据视图：`tushare_current_cb_daily`
- 原始数据表：`tushare_raw_record`（`api_name=cb_daily`）
- REST：`GET /api/v1/data/datasets/cb_daily/records`
- 接口 REST：`GET /api/v1/data/interfaces/cb_daily/records`
...
| `pre_close` | `float` | `NUMERIC` | 昨收盘价(元) |
| `open` | `float` | `NUMERIC` | 开盘价(元) |
| `high` | `float` | `NUMERIC` | 最高价(元) |
| `low` | `float` | `NUMERIC` | 最低价(元) |
| `close` | `float` | `NUMERIC` | 收盘价(元) |
| `change` | `float` | `NUMERIC` | 涨跌(元) |
| `pct_chg` | `float` | `NUMERIC` | 涨跌幅(%) |
| `vol` | `float` | `NUMERIC` | 成交量(手) |
| `amount` | `float` | `NUMERIC` | 成交金额(万元) |
| `bond_value` | `float` | `NUMERIC` | 纯债价值 |
| `bond_over_rate` | `float` | `NUMERIC` | 纯债溢价率(%) |
| `cb_value` | `float` | `NUMERIC` | 转股价值 |
| `cb_over_rate` | `float` | `NUMERIC` | 转股溢价率(%) |
