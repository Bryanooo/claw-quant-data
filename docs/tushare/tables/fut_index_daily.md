# `fut_index_daily` 标准化表契约

- 功能：获取南华指数每日行情，指数行情也可以通过 通用行情接口 获取数据．
- PostgreSQL 表：`tushare_norm_fut_index_daily`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=fut_index_daily`）
- REST：`GET /api/v1/data/datasets/fut_index_daily/records`
- 接口 REST：`GET /api/v1/data/interfaces/fut_index_daily/records`
...
|---|---|---|---|
| `ts_code` | `str` | `TEXT` | TS指数代码 |
| `trade_date` | `str` | `DATE` | 交易日 |
| `close` | `float` | `NUMERIC` | 收盘点位 |
| `open` | `float` | `NUMERIC` | 开盘点位 |
| `high` | `float` | `NUMERIC` | 最高点位 |
| `low` | `float` | `NUMERIC` | 最低点位 |
| `pre_close` | `float` | `NUMERIC` | 昨日收盘点 |
| `change` | `float` | `NUMERIC` | 涨跌点 |
