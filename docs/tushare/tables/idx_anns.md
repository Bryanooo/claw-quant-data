...
- PostgreSQL 表：`tushare_norm_idx_anns`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=idx_anns`）
- REST：`GET /api/v1/data/datasets/idx_anns/records`
- 接口 REST：`GET /api/v1/data/interfaces/idx_anns/records`
...
| `url` | `str` | `TEXT` | 链接 |
| `source` | `str` | `TEXT` | 来源 |
| `type` | `str` | `TEXT` | 类型(指数发布、指数修订、指数更名、其他） |
