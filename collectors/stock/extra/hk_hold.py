...
logger = logging.getLogger("collector.hk_hold")

_FIELDS = ["code", "trade_date", "ts_code", "name", "vol", "ratio", "exchange"]
_PK = ["ts_code", "trade_date"]


class HkHoldCollector:
    INTERFACE_NAME = "hk_hold"
    TABLE_NAME = "hk_hold"
    CORE_FIELDS = _FIELDS
    ...
    def fetch(self, **params):
        return self.pro.hk_hold(**params)
    ...
    def collect_all_history(self, start_date="20200101", end_date="20260513"):
        ...
        while d <= e:
            ...
            d += timedelta(days=1)
            time.sleep(0.3)
        logger.info(f"✅ hk_hold: 历史补齐 {total} 行")
        return total
