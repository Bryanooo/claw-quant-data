...
- 功能：广州民间借贷利率
- PostgreSQL 表：`tushare_norm_gz_index`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=gz_index`）
- REST：`GET /api/v1/data/datasets/gz_index/records`
- 接口 REST：`GET /api/v1/data/interfaces/gz_index/records`
...
|---|---|---|---|
| `date` | `str` | `DATE` | 日期 |
| `d10_rate` | `float` | `NUMERIC` | 小额贷市场平均利率（十天） （单位：%，下同） |
| `m1_rate` | `float` | `NUMERIC` | 小额贷市场平均利率（一月期） |
| `m3_rate` | `float` | `NUMERIC` | 小额贷市场平均利率（三月期） |
| `m6_rate` | `float` | `NUMERIC` | 小额贷市场平均利率（六月期） |
| `m12_rate` | `float` | `NUMERIC` | 小额贷市场平均利率（一年期） |
| `long_rate` | `float` | `NUMERIC` | 小额贷市场平均利率（长期） |
