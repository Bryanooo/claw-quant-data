...
- PostgreSQL 表：`tushare_norm_hk_daily`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=hk_daily`）
- REST：`GET /api/v1/data/datasets/hk_daily/records`
- 接口 REST：`GET /api/v1/data/interfaces/hk_daily/records`
...
| `vol` | `float` | `NUMERIC` | 成交量(股) |
| `amount` | `float` | `NUMERIC` | 成交额(元) |
