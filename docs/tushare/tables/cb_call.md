...
- PostgreSQL 表：`tushare_norm_cb_call`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=cb_call`）
- REST：`GET /api/v1/data/datasets/cb_call/records`
- 接口 REST：`GET /api/v1/data/interfaces/cb_call/records`
...
| `call_type` | `str` | `TEXT` | 赎回类型：到赎、强赎 |
| `is_call` | `str` | `TEXT` | 是否赎回：已满足强赎条件、公告提示强赎、公告实施强赎、公告到期赎回、公告不强赎 |
| `ann_date` | `str` | `DATE` | 公告/提示日期 |
| `call_date` | `str` | `DATE` | 赎回日期 |
| `call_price` | `float` | `NUMERIC` | 赎回价格(含税，元/张) |
| `call_price_tax` | `float` | `NUMERIC` | 赎回价格(扣税，元/张) |
| `call_vol` | `float` | `NUMERIC` | 赎回债券数量(张) |
| `call_amount` | `float` | `NUMERIC` | 赎回金额(万元) |
| `payment_date` | `str` | `DATE` | 行权后款项到账日 |
| `call_reg_date` | `str` | `DATE` | 赎回登记日 |
