...
- PostgreSQL 表：`tushare_norm_etf_sh_cons`
- 契约版本：`1`
...
- 当前数据视图：`tushare_current_etf_sh_cons`
- 原始数据表：`tushare_raw_record`（`api_name=etf_sh_cons`）
- REST：`GET /api/v1/data/datasets/etf_sh_cons/records`
- 接口 REST：`GET /api/v1/data/interfaces/etf_sh_cons/records`
...
| `sub_flag` | `str` | `TEXT` | 现金替代标志：允许/必须 |
| `cpr` | `float` | `NUMERIC` | 申购现金替代溢价比率（%） |
| `rdr` | `float` | `NUMERIC` | 赎回现金替代折价比率（%） |
| `sca` | `float` | `NUMERIC` | 替代金额(单位：人民币元) |
