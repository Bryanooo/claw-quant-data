...
日期范围响应如果恰好命中疑似上游行数上限，采集器不会直接接受，也不会关闭完整性
保护；它会丢弃不确定响应并递归拆分日期窗口。只有每个叶子窗口均未触顶、返回日期
未越界后，才合并写入并记录 `adaptive_date_window_exhaustion` 完整性证据。单日仍
触顶时继续失败关闭，等待更明确的接口契约或分页能力。
