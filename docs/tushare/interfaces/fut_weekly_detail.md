...
- Token 权限：**有权限**（接口返回成功）
- 官方权限要求：600积分可调取，单次最大获取4000行数据，积分越高频次越高，5000积分以上正常调取不受限制 数据来源：中国证监会，本数据由Tushare社区成员CE完成规划和采集
- 官方文档：[doc 216](https://tushare.pro/document/2?doc_id=216)
...
强类型标准表 `tushare_norm_fut_weekly_detail`。标准表字段、类型、日期列、系统血缘字段和
稳定性规则见[标准化表契约](../tables/fut_weekly_detail.md)。转换失败记录进入
