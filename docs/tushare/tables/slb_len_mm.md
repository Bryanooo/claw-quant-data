...
- 功能：做市借券交易汇总
- PostgreSQL 表：`tushare_norm_slb_len_mm`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=slb_len_mm`）
- REST：`GET /api/v1/data/datasets/slb_len_mm/records`
- 接口 REST：`GET /api/v1/data/interfaces/slb_len_mm/records`
...
|---|---|---|---|
| `trade_date` | `str` | `DATE` | 交易日期（YYYYMMDD） |
| `ts_code` | `str` | `TEXT` | 股票代码 |
| `name` | `str` | `TEXT` | 股票名称 |
| `ope_inv` | `float` | `NUMERIC` | 期初余量(万股) |
| `lent_qnt` | `float` | `NUMERIC` | 融出数量(万股) |
| `cls_inv` | `float` | `NUMERIC` | 期末余量(万股) |
| `end_bal` | `float` | `NUMERIC` | 期末余额(万元) |
