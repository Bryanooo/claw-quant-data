...
- 功能：获取沪深交易所债券大宗交易数据
- PostgreSQL 表：`tushare_norm_bond_blk`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=bond_blk`）
- REST：`GET /api/v1/data/datasets/bond_blk/records`
- 接口 REST：`GET /api/v1/data/interfaces/bond_blk/records`
...
| `vol` | `float` | `NUMERIC` | 累计成交数量（万股/万份/万张/万手） |
| `amount` | `float` | `NUMERIC` | 累计成交金额（万元） |
