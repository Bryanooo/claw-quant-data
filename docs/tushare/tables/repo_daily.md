...
- 功能：债券回购日行情
- PostgreSQL 表：`tushare_norm_repo_daily`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=repo_daily`）
- REST：`GET /api/v1/data/datasets/repo_daily/records`
- 接口 REST：`GET /api/v1/data/interfaces/repo_daily/records`
...
| `repo_maturity` | `str` | `TEXT` | 期限品种 |
| `pre_close` | `float` | `NUMERIC` | 前收盘(%) |
| `open` | `float` | `NUMERIC` | 开盘价(%) |
| `high` | `float` | `NUMERIC` | 最高价(%) |
| `low` | `float` | `NUMERIC` | 最低价(%) |
| `close` | `float` | `NUMERIC` | 收盘价(%) |
| `weight` | `float` | `NUMERIC` | 加权价(%) |
| `weight_r` | `float` | `NUMERIC` | 加权价(利率债)(%) |
| `amount` | `float` | `NUMERIC` | 成交金额(万元) |
| `num` | `int` | `BIGINT` | 成交笔数(笔) |
