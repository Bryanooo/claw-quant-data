...
- PostgreSQL 表：`tushare_norm_sw_daily`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=sw_daily`）
- REST：`GET /api/v1/data/datasets/sw_daily/records`
- 接口 REST：`GET /api/v1/data/interfaces/sw_daily/records`
...
|---|---|---|---|
| `ts_code` | `str` | `TEXT` | 指数代码 |
| `trade_date` | `str` | `DATE` | 交易日期 |
| `name` | `str` | `TEXT` | 指数名称 |
| `open` | `float` | `NUMERIC` | 开盘点位 |
| `low` | `float` | `NUMERIC` | 最低点位 |
| `high` | `float` | `NUMERIC` | 最高点位 |
| `close` | `float` | `NUMERIC` | 收盘点位 |
| `change` | `float` | `NUMERIC` | 涨跌点位 |
| `pct_change` | `float` | `NUMERIC` | 涨跌幅 |
| `vol` | `float` | `NUMERIC` | 成交量（万股） |
| `amount` | `float` | `NUMERIC` | 成交额（万元） |
| `pe` | `float` | `NUMERIC` | 市盈率 |
| `pb` | `float` | `NUMERIC` | 市净率 |
| `float_mv` | `float` | `NUMERIC` | 流通市值（万元） |
