# `stk_account_old` — 股票账户开户数据（旧）

- 分类：股票数据/参考数据
- 功能：获取股票账户开户数据旧版格式数据，数据从2008年1月开始，到2015年5月29，新数据请通过 股票开户数据 获取。
- Token 权限：**接口无效/已下线**（Tushare 不再识别该 api_name）
- 官方权限要求：600积分可调取，具体请参阅 积分获取办法
- 官方文档：[doc 165](https://tushare.pro/document/2?doc_id=165)
