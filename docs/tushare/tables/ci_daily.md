...
- PostgreSQL 表：`tushare_norm_ci_daily`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=ci_daily`）
- REST：`GET /api/v1/data/datasets/ci_daily/records`
- 接口 REST：`GET /api/v1/data/interfaces/ci_daily/records`
...
| `pre_close` | `float` | `NUMERIC` | 昨日收盘点位 |
