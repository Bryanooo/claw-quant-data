...
- 功能：获取上市公司定期财务审计意见数据
- PostgreSQL 表：`tushare_norm_fina_audit`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=fina_audit`）
- REST：`GET /api/v1/data/datasets/fina_audit/records`
- 接口 REST：`GET /api/v1/data/interfaces/fina_audit/records`
...
| `ann_date` | `str` | `DATE` |  |
| `end_date` | `str` | `DATE` |  |
| `audit_result` | `str` | `TEXT` |  |
| `audit_fees` | `float` | `NUMERIC` |  |
| `audit_agency` | `str` | `TEXT` |  |
| `audit_sign` | `str` | `TEXT` |  |
