...
- 功能：国债实际长期利率平均值
- PostgreSQL 表：`tushare_norm_us_trltr`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=us_trltr`）
- REST：`GET /api/v1/data/datasets/us_trltr/records`
- 接口 REST：`GET /api/v1/data/interfaces/us_trltr/records`
...
|---|---|---|---|
| `date` | `str` | `DATE` | 日期 |
| `ltr_avg` | `float` | `NUMERIC` | 实际平均利率LT Real Average (10> Yrs) |
