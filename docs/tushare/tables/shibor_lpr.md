...
- 功能：LPR贷款基础利率
- PostgreSQL 表：`tushare_norm_shibor_lpr`
- 契约版本：`1`
...
- 原始数据表：`tushare_raw_record`（`api_name=shibor_lpr`）
- REST：`GET /api/v1/data/datasets/shibor_lpr/records`
- 接口 REST：`GET /api/v1/data/interfaces/shibor_lpr/records`
...
|---|---|---|---|
| `date` | `str` | `DATE` | 日期 |
| `1y` | `float` | `NUMERIC` | 1年贷款利率 |
| `5y` | `float` | `NUMERIC` | 5年贷款利率 |
